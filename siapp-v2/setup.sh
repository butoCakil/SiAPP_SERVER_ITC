...
APP_PORT=$(grep "^APP_PORT=" .env | cut -d= -f2); APP_PORT=${APP_PORT:-8080}
MQTT_PORT=$(grep "^MQTT_PORT=" .env | cut -d= -f2); MQTT_PORT=${MQTT_PORT:-1883}

if ss -tlnp | grep -q ":${APP_PORT} "; then
  echo ""
  echo "  PERINGATAN: Port ${APP_PORT} sudah dipakai oleh proses lain."
  echo "  Ubah APP_PORT di .env ke port lain, misalnya: APP_PORT=9080"
...
if ss -tlnp | grep -q ":${MQTT_PORT} "; then
  ...
  echo "  Ubah MQTT_PORT di .env ke port lain, misalnya: MQTT_PORT=1884"
  echo "  Kemudian jalankan: docker compose up -d"
  exit 1
fi
