...
.device-grid.compact-view .device-card { display: none; }
.device-grid.compact-view .device-card-compact { display: flex !important; }
.device-card-compact {
    display: none;
...
.dcc-row1 { display:flex; align-items:center; justify-content:space-between; gap:6px; }
.dcc-id { font-size:13px; font-weight:700; font-family:'Fira Code',monospace; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
.dcc-rssi-wrap { height:6px; background:#eee; border-radius:4px; overflow:hidden; }
.dcc-rssi-fill { height:100%; border-radius:4px; transition:width 0.4s; }
.dcc-row2 { display:flex; align-items:center; gap:6px; font-size:10px; color:#555; flex-wrap:wrap; }
.dcc-badge { background:#f0f0f0; border-radius:6px; padding:1px 6px; font-size:10px; font-weight:600; color:#333; }
...
.dcc-rssi-row { display:flex; align-items:center; gap:4px; }
.dcc-row3 { display:flex; align-items:center; justify-content:space-between; margin-top:2px; }
.dcc-ssid { font-size:9px; color:#888; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:120px; }
...
.device-card:hover { transform: translateY(-5px); box-shadow: var(--shadow-hover); }
.device-card.is-online  { border-color: #06de72; }
.device-card.is-offline { border-color: #ff3b3b; }

.dc-header { display:flex; justify-content:space-between; align-items:flex-start; gap:8px; }
.dc-title { flex:1; }
.dc-id { font-size:1.1em; font-weight:700; font-family:'Fira Code',monospace; display:flex; align-items:center; gap:6px; }
.dc-fw { background:#ffd500; border-radius:8px; padding:1px 7px; font-size:10px; font-weight:600; box-shadow:inset 0 0 4px rgba(0,0,0,0.2); }
.dc-info { background:#00d0ff; border-radius:8px; text-align:center; font-size:11px; padding:1px 8px; margin-top:3px; display:inline-block; box-shadow:inset 0 0 4px rgba(0,0,0,0.15); }
...
.dc-dot { width:26px; height:26px; border-radius:50%; flex-shrink:0; margin-top:2px; }
...
.ram-bar { width:100%; height:9px; border-radius:8px; overflow:hidden; background:linear-gradient(to right,#00aaff,#00ff66 50%,#ffee00 70%,#ff8800 85%,#ff0000); position:relative; }
.ram-bar::after { content:""; position:absolute; top:0; right:0; height:100%; width:calc(100% - var(--ram-pct, 0%)); background:#e0e0e0; transition:width 0.5s ease; }
...
.dc-actions-top { display:flex; gap:5px; margin-top:8px; align-items:center; }
.dc-section-title { font-size:11px; font-weight:700; color:#444; margin-top:10px; display:flex; align-items:center; gap:6px; }

.btn-xs-round { width:28px; height:28px; border-radius:50%; border:none; cursor:pointer; font-size:13px; display:flex; align-items:center; justify-content:center; box-shadow:inset 0 2px 4px rgba(255,255,255,0.4),inset 0 -2px 4px rgba(0,0,0,0.2),0 3px 7px rgba(0,0,0,0.2); transition:transform 0.2s,filter 0.2s; }
...
.dc-btn { flex:1; padding:5px 4px; border:none; border-radius:12px; font-size:12px; font-weight:600; color:#fff; cursor:pointer; box-shadow:inset 0 2px 5px rgba(255,255,255,0.35),inset 0 -2px 5px rgba(0,0,0,0.2),0 3px 8px rgba(0,0,0,0.22); transition:transform 0.2s,filter 0.2s; }
...
.dc-last-setting { font-size:10px; background:#f5f5f5; border-radius:8px; padding:6px 8px; margin-top:6px; color:#444; }
.dc-viewlog { font-size:11px; margin-top:6px; }
.dc-viewlog a { color:#555; text-decoration:none; }
.dc-viewlog a:hover { color:#007bff; }

.global-actions { display:flex; gap:10px; flex-wrap:wrap; margin-bottom:18px; }
.btn-global { padding:7px 18px; border:none; border-radius:20px; font-weight:600; font-size:13px; color:#fff; cursor:pointer; box-shadow:inset 0 2px 5px rgba(255,255,255,0.3),0 4px 10px rgba(0,0,0,0.2); transition:transform 0.2s,filter 0.2s; }
...
<div class="summary-bar" style="justify-content: space-between;">
    ...
    <div class="summary-badge"><div class="dot-green"></div>Online: <strong id="cnt-online">{{ $onlineCount }}</strong></div>
    <div class="summary-badge"><div class="dot-red"></div>Offline: <strong id="cnt-offline">{{ $offlineCount }}</strong></div>
    <div class="summary-badge">📊 Total: <strong id="cnt-total">{{ $onlineCount + $offlineCount }}</strong></div>
    <div class="summary-badge">🔄 Refresh: <strong><span id="countdown">60</span>s</strong></div>
    <div class="summary-badge text-muted" style="font-size:11px;">Terakhir: <span id="last-refresh">--:--:--</span></div>
</div>
...
function getColor(pct) {
    const stops = [[0,[255,0,0]],[20,[255,50,0]],[40,[255,150,0]],[60,[255,255,0]],[80,[37,211,102]],[100,[0,170,255]]];
...
async function editLabel(deviceId, currentLabel) {
    ...
    else { alert('Gagal update label: ' + (json.message ?? 'Unknown error')); }
}
...
// ── handleCmd per-device dengan modal ──
...
async function sendAll(cmdKey) {
    ...
    try {
        ...
        const data = await res.json();
        data.forEach(d => { pollData[d.device_id] = d; });
    } catch(e) { alert('Gagal ambil status device: ' + e.message); return; }
    ...
    const sendPromises = deviceStates.filter(d => !d.skip).map(d => sendCmd(d.device_id, cmdKey));
    ...
    cmdModalTimer = setInterval(async () => {
        ...
        if (stillWaiting.length === 0) { clearInterval(cmdModalTimer); updateCmdSummary(deviceStates); return; }
        let pollNow = {};
        try {
            const res  = await fetch('/api-internal/device-poll-status');
            const data = await res.json();
        ...
        stillWaiting.forEach(d => {
            ...
            if (!current) return;
            processPollingResult(d, current,
                (msg) => { d.state='ok'; updateCmdRow(d.device_id,'ok',msg); },
                ()    => { d.state='fail'; updateCmdRow(d.device_id,'fail','❌ Timeout'); },
                now
            );
        });
        updateCmdSummary(deviceStates);
        if (deviceStates.filter(d => d.state === 'pending').length === 0) clearInterval(cmdModalTimer);
    }, CMD_POLL_MS);
}
...
async function refreshGrid() {
    ...
    try {
        const res  = await fetch('/device/cards', { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
        const data = await res.json();
...
window.addEventListener('load', () => { updateBars(); updateLastRefresh(); });
...
async function deleteDevice(deviceId) {
    try {
        ...
        const data = await res.json();
        if (data.status === "ok") { document.querySelector(`[data-device-id="${deviceId}"]`)?.remove(); }
        else { alert("Gagal hapus device."); }
...
async function loadSparkline(deviceId) {
    ...
    } catch(e) { console.warn('Sparkline error ' + deviceId, e); }
}

function loadAllSparklines() {
    document.querySelectorAll('[data-device-id]').forEach(el => loadSparkline(el.dataset.deviceId));
}
